...
import InscripcionesTable from "./InscripcionesTable"; // Nuevo componente
...
export default function Inscripciones() {
    ...
    const updateEstado = useCallback(async (id, nuevoEstado) => {
        try {
            ...
            // Nota: Al mover esto a un callback, necesitamos que 'inscripciones', 'modalities' sean frescos.
            // OJO: useCallback con dependencias [inscripciones, modalities] hará que la función cambie
            // y la tabla se re-renderice cuando cambien las inscripciones.
            // Para evitar stale closures, usaremos dependencia.
            // Sin embargo, queremos que NO se renderice al tipear en search.
            // Como 'search' cambia, 'inscripciones' NO cambia (hasta el debounce).
            // Así que es seguro incluir 'inscripciones' aquí.
    ...
    }, [inscripciones, modalities, fetchInscripciones, showToast]);

    const handleViewDetail = useCallback((ins) => {
        setSelectedInscripcion(ins);
        setIsModalOpen(true);
    }, []);

    const handleEdit = useCallback((ins) => {
